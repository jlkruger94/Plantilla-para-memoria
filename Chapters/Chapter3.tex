...
Como se muestra en la figura \ref{fig:arq_bloques}, la capa de aplicación está constituida por los componentes de software encargados de gestionar cada una de las funcionalidades del sistema.
...
\begin{itemize}
	...
	\item Servidor web embebido: permite la comunicación con el sistema de control del cultivo a través de la red. Este servidor permite monitorear, configurar y controlar el sistema desde cualquier dispositivo con acceso a la red.
...
La arquitectura sigue el patrón modular con módulos de software que ejecutan tareas de forma ordenada. El ciclo comienza en \texttt{main.c} con la inicialización del sistema a través del componente \texttt{Administrator}.

\subsubsection*{Inicialización del sistema}
Como se ilustra en la figura \ref{fig:flujo:sistema}, la función \texttt{app\_main()} crea la tarea \texttt{admi\-nistrator\_callback} con alta prioridad, que actúa como un coordinador general que gestiona la interacción entre módulos y subsistemas.
...
Una vez inicializado, el \texttt{Administrator} ejecuta un bucle principal que se repite cada segundo y realiza las funciones detalladas en la figura \ref{fig:flujo:sistema}:
...
El diseño del PCB se realizó en KiCad. La figura \ref{fig:esquematico:base} muestra el acondicionamiento de señal para los módulos de TDS y pH, junto con los circuitos de protección y regulación de tensión. La figura \ref{fig:esquematico:reles} muestra la interfaz de salida para la bomba de agua. Los demás actuadores emplean un circuito similar. Ambos esquemas forman parte de la placa centralizadora.

\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{./Figures/circuit_sensor.png}
\caption{Vista parcial del circuito de la placa centralizadora que muestra la interconexión de los módulos de TDS y pH con el microcontrolador y el circuito de acondicionamiento de señal.}
\label{fig:esquematico:base}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{./Figures/pump_hw.png}
\caption{Circuito de control de la bomba con aislamiento galvánico y relé de potencia.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.46\textwidth}
        \centering
        \includegraphics[width=\textwidth]{./Figures/montaje/montaje_final_1.png}
...
\begin{itemize}
    ...
    \item Luminosidad: sensor digital BH1750 con rango de 1 a 65\,535\,lx. El amplio rango dinámico permite una medición precisa de la iluminación artificial requerida para el ciclo día-noche del cultivo, con interfaz I2C que optimiza la integración con el microcontrolador.
\end{itemize}
...
\begin{itemize}
    ...
    \item Para calibrar el sensor de humedad, primero se realizó una medición con el sensor en sustrato seco y se estableció el valor en 0 \%. Luego se sumergió completamente en agua y se ajustó el valor a 100 \%.
...
El núcleo \texttt{App} contiene dos componentes principales: \texttt{Navbar} y \texttt{Router}. Este último gestiona todas las vistas: \texttt{Login} para autenticación, \texttt{Dashboard} con gráficos y estados de sensores, \texttt{Actuators} para control de actuadores, \texttt{Config} para ajustes avanzados, y \texttt{Logs} con el historial de alertas. Además, se incluye el módulo \texttt{AuthProvider}, encargado de gestionar y proporcionar a los distintos componentes las credenciales de acceso al servidor embebido.
...
La seguridad de las comunicaciones se garantiza mediante un sistema de autenticación basado en tokens. Como se ilustra en la figura \ref{fig:login_sec}, cuando el usuario ingresa las credenciales, el componente \texttt{Login} envía una petición \texttt{POST} a la API. Si las credenciales son válidas, la API responde con una clave de acceso que el \texttt{AuthProvider} almacena en el \texttt{localStorage} del navegador. Finalmente, el usuario es redirigido al \texttt{Dashboard}.
...
\subsection{Monitoreo de sensores}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{./Figures/ui/sensores.png}
    \caption{Diagrama de secuencia de la vista de monitoreo de sensores.}
    \label{fig:sensores}
\end{figure}

\subsection{Control de actuadores}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{./Figures/ui/actuadores.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{./Figures/ui/configuracion.png}
