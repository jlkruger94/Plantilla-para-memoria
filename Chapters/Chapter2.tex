...
\begin{figure}[H]
\centering
\includegraphics[scale=.35]{./Figures/devkit.png}
...
\begin{itemize}
	...
	\item Salida analógica que varía, entre 0 y 5 VDC, con el pH del líquido.
...
El medidor de sólidos disueltos totales o TDS (del inglés, \textit{Total Dissolved Solids}) Meter 1.0, que se muestra en la figura \ref{fig:tds}, es un sensor electrónico diseñado para medir la cantidad total de sustancias orgánicas e inorgánicas disueltas en un líquido, lo que se expresa típicamente en partes por millón (ppm) o miligramos por litro (mg/l) \cite{TDS}. 

\begin{figure}[H]
    \centering
    \includegraphics[scale=.4]{./Figures/tds.png}
...
El DS18B20, que se muestra en la figura \ref{fig:ds18b20}, es un sensor de temperatura digital que proporciona mediciones en grados Celsius con una resolución configurable de 9 a 12 bits \cite{DS18B20}. %Este sensor se comunica a través de un bus 1-Wire, lo que significa que solo requiere una línea de datos (además de tierra) para la comunicación con un micontrolador. Cada DS18B20 tiene un código de serie único de 64 bits grabado en fábrica, lo que permite que múltiples sensores funcionen en el mismo bus 1-Wire, lo que permite la creación de redes de sensores de temperatura distribuidos.
...
Raspberry Pi es una plataforma de computación embebida basada en arquitectura ARM \cite{arm}, capaz de ejecutar sistemas operativos Linux como Raspberry Pi OS \cite{raspi}. Dispone de interfaces GPIO \cite{gpio}, I²C \cite{I2C}, SPI \cite{spi} y UART \cite{uart} que permiten la interacción directa con sensores, actuadores y periféricos externos. Su bajo consumo energético, soporte para redes Ethernet/Wi-Fi \cite{ethernet}, \cite{wifi} y capacidad de procesamiento la convierten en una herramienta versátil para el desarrollo, prueba e implementación de sistemas embebidos y aplicaciones IoT \cite{raspi}.
