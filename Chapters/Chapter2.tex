...
\begin{figure}[H]
\centering
\includegraphics[scale=.4]{./Figures/devkit.png}
...
El medidor de sólidos disueltos totales (TDS, del inglés \textit{Total Dissolved Solids}) Meter 1.0 (figura \ref{fig:tds}) es un sensor electrónico diseñado para medir la cantidad total de sustancias orgánicas e inorgánicas disueltas en un líquido, lo que se expresa típicamente en partes por millón (ppm) o miligramos por litro (mg/l) \cite{TDS}. 
...
Raspberry Pi es una plataforma de computación embebida basada en arquitectura ARM, capaz de ejecutar sistemas operativos Linux como Raspberry Pi OS. Dispone de interfaces GPIO, I²C, SPI y UART que permiten la interacción directa con sensores, actuadores y periféricos externos. Su bajo consumo energético, soporte para redes Ethernet/Wi-Fi y capacidad de procesamiento la convierten en una herramienta versátil para el desarrollo, prueba e implementación de sistemas embebidos y aplicaciones IoT \cite{raspi}.
...
\subsection{Docker}
Docker es una plataforma de código abierto que permite empaquetar, distribuir y ejecutar aplicaciones en entornos aislados llamados contenedores. Estos contenedores garantizan que el software funcione de forma consistente en cualquier entorno, facilitando la portabilidad, la escalabilidad y la automatización del despliegue de aplicaciones \cite{docker}.
